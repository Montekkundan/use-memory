...
async function tryHandleSlackLinkCommand(
  ...
  if (result.ok) {
    await ctx.thread.post(
      "Your Slack account is now linked to Use Memory. Mentions and DMs will use your profile and integrations.",
    );
    return true;
  }

  const reason = result.reason === "expired"
    ? "That link code has expired. Generate a new one in Use Memory → Integrations."
    : "That link code is invalid. Generate a fresh code in Use Memory → Integrations.";
...
async function buildSlackTurn(ctx: SlackContext, message: SlackMessage) {
  ...
  if (prior.length > 0) {
    const transcript = prior
      .map((m) => `${m.isMe ? "Use Memory" : (m.user ?? "user")}: ${m.markdown}`)
      .join("\n");
  ...
  if (!linked) {
    const linkUrl = process.env.BETTER_AUTH_URL
      ...
      : "Use Memory → Integrations";
    context.push(
      `This Slack account is not linked to a Use Memory profile yet. Open ${linkUrl}, generate a link code, then message \`link <code>\` here.`,
    );
  }
